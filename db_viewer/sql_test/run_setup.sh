#!/usr/bin/env bash
# Rerunnable script to set up mock tables and data for db_viewer testing.
# Usage:
#   ORA_USER=LEARN1 ORA_PASS=Welcome ORA_HOST=localhost ORA_PORT=1521 ORA_SERVICE=XE ./db_viewer/sql_test/run_setup.sh
set -euo pipefail
...
SQL_FILE="${SCRIPT_DIR}/setup_mock_data.sql"
...
LOG_FILE="${LOG_DIR}/setup_mock_data_${TS}.log"
TMP_SQL="${LOG_DIR}/_setup_tmp_${TS}.sql"

cat >"${TMP_SQL}" <<EOSQL
SET ECHO ON FEEDBACK ON SERVEROUTPUT ON LINESIZE 200 PAGESIZE 100 TRIMSPOOL ON VERIFY OFF
SPOOL ${LOG_FILE}
PROMPT === Running setup_mock_data.sql ===
@${SQL_FILE}
SPOOL OFF
EXIT
EOSQL
...
echo "[OK] Mock data setup completed. Log: ${LOG_FILE}"
